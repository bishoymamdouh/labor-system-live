...
// 1. Sidebar link
if (!content.includes('id="menu-notifications"')) {
    content = content.replace('<li id="menu-reports"', '<li id="menu-notifications" class="admin-only"><i class="fas fa-bell"></i> إدارة الإشعارات</li>\n                <li id="menu-reports"');
}
...
if (!content.includes('id="view-notifications"')) {
    content = content.replace('<div id="view-users"', notifHtml + '\n        <div id="view-users"');
}
