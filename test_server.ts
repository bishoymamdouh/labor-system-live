...
const kv = await Deno.openKv(Deno.env.get("DENO_DEPLOYMENT_ID") ? undefined : "./database.sqlite");
    ...
    if (url.pathname.startsWith("/api/")) {
        ...
        if (collection === "allRecordsDetails" && method === "GET") {
            ...
            const detailedRecords = records.map(r => ({
                ...r,
                supervisorName: usersMap[r.supervisorId] || '??? ?????',
                engineerName: usersMap[r.engineerId] || '??? ?????',
